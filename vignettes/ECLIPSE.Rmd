...
for (i in seq_along(transformed_SEs)) {
    ...
    hist(sigs, main = names(transformations)[[i]], xlab = "Transformed Signal", breaks = 50)
    qqnorm(sigs, main = names(transformations)[[i]])
    plot_enhancer_curve(transformed_SEs[[i]], factor_label = "H3K27ac")
}
dev.off()
...
for (i in seq_along(transformed_SEs_width_norm)) {
    ...
    hist(sigs, main = names(transformations)[[i]], xlab = "Transformed Signal", breaks = 50)
    qqnorm(sigs, main = names(transformations)[[i]])
    plot_enhancer_curve(transformed_SEs_width_norm[[i]], factor_label = "H3K27ac")
}
dev.off()
```
