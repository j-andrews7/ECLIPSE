...
Most often, this is done through some some of merging processm sometimes requiring overlap in some arbitrary number or proportion of samples.
...
While it's possible to call SEs for each sample in a group individually and merging, it's tempting to simplify the process by defining a consensus set of peaks and combining signal for all replicates into a single BAM and calling on that.
...
peaks_actB1 <- readBed(peaks_actB1_path)

treat_actB1 <- BamFile(actB1_path)
control_actB1 <- BamFile(control_actB1_path)

actB1_enhancers <- run_rose(treatment = treat_actB1, control = control_actB1, peaks = peaks_actB1)
```
