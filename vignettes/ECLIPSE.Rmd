...
We have closely examined and tested the ROSE source code and have made the following observations:
 ...
 3. Outliers have an outsized impact on the signal ranking and final classifications in ROSE.
   - To elaborate, if you remove the top 5 or 10 regions by signal, the ranking of the remaining regions changes dramatically and the threshold for super-enhancer classification is much different. This results in non-robust classifications whereby you can easily get 100s more/less SEs from sample to sample of the exact same tissue or cell type just depending on how dramatic the outliers are.
...
winsorized_minmax_scale <- function(x, upper_quantile = 0.999, lower_quantile = 0.01) {
  # 1) Identify cutoffs
  q_high <- quantile(x, probs = upper_quantile, na.rm = TRUE)
  q_low  <- quantile(x, probs = lower_quantile, na.rm = TRUE)
  
  # 2) Winsorize
  x_winsor <- pmax(pmin(x, q_high), q_low)
  
  # 3) Min–Max scaling
  x_min <- min(x_winsor, na.rm = TRUE)
  x_max <- max(x_winsor, na.rm = TRUE)
...
log_minmax_transform <- function(x) {
  ...
  # 2) Min–Max scale
  x_min <- min(x_log, na.rm = TRUE)
  x_max <- max(x_log, na.rm = TRUE)
...
asinh_minmax_transform <- function(x, scale_param = 1) {
  ...
  # 2) Min–Max scale
  x_min <- min(x_asinh, na.rm = TRUE)
  x_max <- max(x_asinh, na.rm = TRUE)
...
robust_log2_z_transform <- function(x) {
  (log2(x) - median(log2(x), na.rm = TRUE)) / mad(log2(x), na.rm = TRUE)
}
...
Now for each transformation function, we'll apply it to the signal to the signal column and plot the histograms and qqplots.
...
transformed_sigs <- lapply(transformations, function(f) f(sig))

# Plot histograms and qqplots in PDF with new page for each transformation, both plots on one page
pdf("transformed_signal_distributions.NaiveB1.pdf", width = 10, height = 6)
par(mfrow = c(1, 2))
for (i in seq_along(transformed_sigs)) {
  hist(transformed_sigs[[i]], main = names(transformations)[[i]], xlab = "Transformed Signal", breaks = 50)
  qqnorm(transformed_sigs[[i]], main = names(transformations)[[i]])
}
dev.off()
...
# Plot histograms and qqplots in PDF with new page for each transformation, both plots on one page
pdf("transformed_signal_distributions.NaiveB1.width_norm.pdf", width = 10, height = 6)
par(mfrow = c(1, 2))
