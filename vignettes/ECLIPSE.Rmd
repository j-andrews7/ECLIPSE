...
ECLIPSE handles these slightly different than BAM files, for which it uses the total number of reads as an additional normalization step, as that's what ROSE does (even though it doesn't really make any sense and you get largely similar results without it given how per-region coverage is calculated).

Given this is new functionality, we can do whatever we want, which is to just calculate coverage for the regions divided by the width of the region. This is simple and fast. In these cases, the assumption is that the input or IgG control signal has already been incorporated (or subtracted or whatever) as appropriate and thus will not be provided.
