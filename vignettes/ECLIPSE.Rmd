...
for (i in seq_along(transformed_SEs)) {
    ...
    hist(sigs, main = names(transformations)[[i]], xlab = "Transformed Signal", breaks = 75)
    p <- plot_qq(ses, "rankby_signal", "normal") + labs(subtitle = (names(transformations)[[i]]), do.raster = TRUE)
    print(p)
    if (min(sigs) > 0) {
        p <- plot_qq(ses, "rankby_signal", "lognormal") + labs(subtitle = (names(transformations)[[i]]), do.raster = TRUE)
        print(p)
    }
    p <- plot_qq(ses, "rankby_signal", "poisson") + labs(subtitle = (names(transformations)[[i]]), do.raster = TRUE)
    print(p)
    plot_enhancer_curve(ses, factor.label = "H3K27ac", do.raster = TRUE, show.legend = FALSE)
}
dev.off()
