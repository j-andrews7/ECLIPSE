...
for (i in seq_along(transformed_SEs)) {
    ...
    p <- ggplot(df, mapping = aes(signal)) + geom_histogram(bins = 75) + labs(title = (names(transformations)[[i]]), x = "Transformed Signal")  + theme_bw()
    p2 <- plot_qq(ses, "rankby_signal", "normal") + labs(subtitle = (names(transformations)[[i]]), do.raster = TRUE)
    p3 <- NULL
    if (min(sigs) > 0) {
        p3 <- plot_qq(ses, "rankby_signal", "lognormal") + labs(subtitle = (names(transformations)[[i]]), do.raster = TRUE)
    }
    p4 <- plot_qq(ses, "rankby_signal", "poisson") + labs(subtitle = (names(transformations)[[i]]), do.raster = TRUE)
    p5 <- plot_enhancer_curve(ses, factor.label = "H3K27ac", do.raster = TRUE, show.legend = FALSE)
